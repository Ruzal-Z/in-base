...
class itartHome(DataMixin, ListView):
    ...
    def get_queryset(self):  # отображаем только статьи где отмечено публикация
        return itart.objects.filter(is_published=True)

'''    
def index(request):
    posts = itart.objects.all()
    ...
    return render(request, 'itart/index.html', context=context)
'''

def about(request):
    return render(request, 'itart/about.html', {'menu': menu, 'title': 'О сайте'})
...
def addpage(request):
    if request.method =='POST':
        ...
        form = AddPostForm()
    return render(request, 'itart/addpage.html', {'form': form, 'menu': menu, 'title': 'Добавление статьи'})
'''
...
def show_post(request, post_slug):
    post = get_object_or_404(itart, slug=post_slug)
    ...
    return  render(request, 'itart/post.html', context=context)
'''
...
class ItartCategory(DataMixin, ListView):
    ...
    def get_queryset(self):
        return itart.objects.filter(cat__slug=self.kwargs['cat_slug'], is_published=True)
...
def show_category(request, cat_id):
    posts = itart.objects.filter(cat_id=cat_id)
